...
DB_NAME = "a3database"
TABLE_NAME = "anime"
MATERIAL_VIEW = "matview"
SAMPLE_TABLE = "sample_table"
SAMPLE_TABLE_VIEW = "matviewsample"
...
def getData():
  ...
  try:
    ...
    postgreSQL_select_Query = 'SELECT * FROM ' + MATERIAL_VIEW
...
def getUniqueValues():
    try:
        ...
        query = "SELECT DISTINCT " + attribute + " FROM " + MATERIAL_VIEW +  \
            " ORDER BY " + attribute + ";"
...
def getScatterData():
    try:
        ...
        query = "SELECT " + x_attribute + "," + y_attribute + " FROM " \
            + SAMPLE_TABLE_VIEW + " WHERE start_airing <= '" \
        ...
        # log date range change, timestamp of action, old date value, new date value
        LOG_FILE = open("userlogs.txt", "a+")
        LOG_FILE.write(",".join(["scatter_update", str(current_time), x_attribute, "\n"]))
        LOG_FILE.close()
    ...
    try:
        ...
        if attribute == "genres":
            for genre in GENRE_LIST:
                query = "SELECT COUNT(*) FROM " + MATERIAL_VIEW + " WHERE '" \
            ...
            query = "SELECT DISTINCT " + attribute + ", SUM(CASE WHEN " \
                + attribute + " IS NOT NULL THEN 1 ELSE 0 END)" + " FROM " \
                    + "(SELECT * FROM " + MATERIAL_VIEW + " WHERE start_airing <= '" + DATE_RANGES["max"] + "'::date) AS daterange" \
        ...
        print("logging data now")
        LOG_FILE = open("userlogs.txt", "a+")
        LOG_FILE.write(",".join(["histogram_update", str(current_time), attribute, "\n"]))
        LOG_FILE.close()
...
def updateTimeRange():
    try:
        ...
        print("logging data now")
        LOG_FILE = open("userlogs.txt", "a+")
        LOG_FILE.write(",".join(["date_range_change", str(current_time), str(old_time), str(DATE_RANGES["max"]), "\n"]))
        LOG_FILE.close()
        response = json.dumps({}) # (3) empty response; just updating variables
        resp = Response(response=response,status=200, mimetype='application/json')
        h = resp.headers
...
def getExtrema():
  try:
    attribute = request.args.get('attribute')
    query = "select min(" + attribute + ") from " + MATERIAL_VIEW # (4) min
    cursor.execute(query)
    minimum = cursor.fetchall()[0][0]

    query = "select max(" + attribute + ") from " + MATERIAL_VIEW # (4) max
    cursor.execute(query)
...
if __name__ == "__main__":
  ...
  try: 
    ...
    query = "select max(start_airing) from " + MATERIAL_VIEW # (4) max
    cursor.execute(query)
    maximum = cursor.fetchall()[0][0]
    # print(maximum)
    ...
    current_time = datetime.datetime.now().strftime("%H:%M:%S")
    LOG_FILE = open("userlogs.txt", "a+")
    LOG_FILE.write("vis_started," + str(current_time) + "\n")
    LOG_FILE.close()
